// default-ap-handler - Convenient tool to handle web+ap on linux.
...
fn main() {
    let uri = std::env::args().last().unwrap_or_else(String::new);
    std::process::exit(match fc_open_uri_inner(&*uri) {
        Ok(_) => 1,
        Err(_) => 0,
    })
}
...
fn fc_open_uri_inner(uri: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    Err(Command::new("xdg-open").arg(fallback).exec().into())
}
